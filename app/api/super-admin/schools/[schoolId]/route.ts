  ...
  try {
    ...
    const school = await prisma.school.findUnique({
      ...
      include: {
        ...
        _count: { select: { classes: true, teachers: true, users: true, auditLogs: true, timetables: true } },
      },
    });
    ...
    const [studentCount, adminUsers, recentAuditLogs] = await Promise.all([
      prisma.student.count({ where: { classGroup: { schoolId } } }),
      prisma.appUser.findMany({
        where: { roleAssignments: { some: { scopeSchoolId: schoolId, role: { key: { in: ["admin", "smt", "hod"] } } } } },
        select: {
          id: true, email: true, displayName: true, createdAt: true,
          roleAssignments: { where: { scopeSchoolId: schoolId }, include: { role: true } },
        },
      }),
    ...
    return NextResponse.json({
      ...school,
      statistics: { students: studentCount, teachers: school._count.teachers, classes: school._count.classes, users: school._count.users },
      adminUsers,
      recentAuditLogs,
    });
  } catch (error) {
    return handleApiError("GET super-admin/schools/[schoolId]", error);
  }
}
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    if (parsed.data.shortCode && parsed.data.shortCode !== existing.shortCode) {
      const duplicate = await prisma.school.findUnique({ where: { shortCode: parsed.data.shortCode } });
      if (duplicate) {
        return NextResponse.json({ error: "A school with this short code already exists" }, { status: 409 });
      }
    }
    ...
    return handleApiError("PATCH super-admin/schools/[schoolId]", error);
  }
}
  ...
  try {
    ...
    const [school, counts] = await Promise.all([
      ...
      prisma.school.findUnique({
        where: { id: schoolId },
        select: { _count: { select: { classes: true, teachers: true, users: true } } },
      }),
    ]);
    ...
    const hasData = (counts?._count.classes ?? 0) > 0 || (counts?._count.teachers ?? 0) > 0 || (counts?._count.users ?? 0) > 0;

    if (hasData) {
      return NextResponse.json(
        { error: "Cannot delete school with existing data. Remove all teachers, users, and classes first.", counts: counts?._count },
        { status: 409 }
      );
    }
    ...
    return handleApiError("DELETE super-admin/schools/[schoolId]", error);
  }
}
