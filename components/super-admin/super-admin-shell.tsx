  ...
  return (
    <div className="relative flex min-h-screen bg-canvas text-foreground">
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(circle at 30% 20%, hsl(var(--accent-violet) / 0.2), transparent 55%), radial-gradient(circle at 80% 0%, hsl(var(--accent-flamingo) / 0.15), transparent 60%), radial-gradient(circle at 60% 80%, hsl(var(--accent-iris) / 0.2), transparent 60%)",
        }}
        aria-hidden
      />
      ...
      <aside className="relative z-10 hidden w-64 flex-col border-r border-[hsl(var(--border-strong))/0.6] bg-[hsl(var(--surface-strong))/0.9] px-6 py-8 shadow-ambient-sm backdrop-blur lg:flex xl:w-72">
        ...
        <div className="mb-10 flex items-center gap-3 text-lg font-semibold">
          <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[hsl(var(--accent-violet))/0.15] text-[hsl(var(--accent-violet))]">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "flex items-center gap-3 rounded-2xl px-4 py-2.5 transition-all duration-200",
                  isActive
                    ? "bg-[hsl(var(--accent-violet))/0.12] text-foreground shadow-ambient-sm"
        ...
        <header className="sticky top-0 z-20 border-b border-[hsl(var(--border))/0.5] bg-[hsl(var(--surface-strong))/0.85] px-6 py-4 backdrop-blur-xl">
          <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-6">
            <div className="flex items-center gap-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[hsl(var(--accent-violet))/0.15]">
